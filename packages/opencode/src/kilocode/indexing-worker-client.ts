...
export namespace IndexingWorker {
  ...
  type Outgoing =
    ...
    | Omit<Extract<Request, { method: "ping" }>, "id">
  ...
  const worker = (directory: string, root: string, hooks: Hooks): Host => {
    ...
    const host: Host = {
      ...
      ping() {
        return call(
          state,
          { type: "request", key, method: "ping", input: { lancedbPath: process.env.KILO_LANCEDB_PATH } },
          (message) => {
            if (message.ok && message.method === "ping") return message.value as PingResult
            throw new Error("Unexpected indexing worker ping response.")
          },
        )
      },
      ...
      async dispose() {
        ...
        try {
          ...
          if (state.hosts.get(key) === host) state.hosts.delete(key)
          if (pool.get(key) === host) pool.delete(key)
          if (state.hosts.size === 0 && !state.stopped) {
