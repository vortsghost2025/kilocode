...
export class LanceDBVectorStore implements IVectorStore {
  ...
  async deletePointsByMultipleFilePaths(filePaths: string[]): Promise<void> {
    ...
    try {
      ...
      const canonicalPaths = filePaths.map((fp) =>
        path.isAbsolute(fp) ? generateRelativeFilePath(fp, workspaceRoot) : path.normalize(fp),
      )

      // Create filter condition for multiple file paths
      const escapedPaths = canonicalPaths.map((fp) => `'${this.escapeSqlString(fp)}'`).join(", ")
