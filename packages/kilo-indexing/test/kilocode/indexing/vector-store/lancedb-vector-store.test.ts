...
describe("LocalVectorStore", () => {
  ...
  describe("SQL Injection Prevention", () => {
    ...
    test("should handle paths with backslashes safely", async () => {
      ...
      // Path is outside the workspace workspacePath ("mock/workspace"),
      // so generateRelativeFilePath normalizes it via path.relative,
      // producing a relative path with ".." segments.
      expect(mockTable.delete).toHaveBeenCalled()
    })

    test("passes workspace-relative paths through unchanged", async () => {
      const relativePath = "src/components/Button.tsx"
      mockTable.delete.mockResolvedValue(undefined)

      await store.deletePointsByFilePath(relativePath)
      const expected = path.normalize(relativePath)

      expect(mockTable.delete).toHaveBeenCalledWith(`\`filePath\` IN ('${expected}')`)
    })

    test("normalizes absolute workspace paths to relative", async () => {
      const absPath = path.resolve(workspacePath, "src/components/Button.tsx")
      const expected = path.normalize("src/components/Button.tsx")
      mockTable.delete.mockResolvedValue(undefined)

      await store.deletePointsByFilePath(absPath)

      expect(mockTable.delete).toHaveBeenCalledWith(`\`filePath\` IN ('${expected}')`)
    })

    test("preserves backslash paths in SQL filter", async () => {
      const backslashPath = "src\\components\\Button.tsx"
      mockTable.delete.mockResolvedValue(undefined)

      await store.deletePointsByFilePath(backslashPath)

      expect(mockTable.delete).toHaveBeenCalledWith(`\`filePath\` IN ('${backslashPath}')`)
    })
  })

  describe("upsert/delete path identity", () => {
    const wsPath = path.resolve("test", "workspace")
    const store2 = new LanceDBVectorStore(wsPath, 768, path.resolve("test", "db"))

    beforeEach(() => {
      resetAllMocks()
    })

    test("relative path passed through upsert and delete is identical", async () => {
      const filename = "src/components/Button.tsx"
      const payload = { filePath: filename, fileHash: "abc", codeChunk: "code", startLine: 1, endLine: 2 }
      const points = [{ id: "123e4567-e89b-12d3-a456-426614174000", vector: [1, 2, 3], payload }]

      await store2.upsertPoints(points)
      const storedData = mockTable.add.mock.calls[0][0]
      const storedFilePath = storedData[0].filePath
      expect(storedFilePath).toBe(filename)

      resetAllMocks()
      mockTable.delete.mockResolvedValue(undefined)
      const absPath = path.resolve(wsPath, filename)
      await store2.deletePointsByFilePath(absPath)
      const expectedDeleteKey = path.normalize(filename)
      const deleteFilter = mockTable.delete.mock.calls[0][0]
      expect(deleteFilter).toBe(`\`filePath\` IN ('${expectedDeleteKey}')`)
    })

    test("backslash relative path and forward-slash relative resolve to same key", async () => {
      const fsPath = "src/components/Button.tsx"
      const bsPath = "src\\components\\Button.tsx"
      const payload = { filePath: fsPath, fileHash: "abc", codeChunk: "code", startLine: 1, endLine: 2 }
      const points = [{ id: "123e4567-e89b-12d3-a456-426614174000", vector: [1, 2, 3], payload }]
      const expectedKey = path.normalize(fsPath)

      resetAllMocks()
      await store2.upsertPoints(points)
      const upsertKey = mockTable.add.mock.calls[0][0][0].filePath
      expect(upsertKey).toBe(fsPath)

      resetAllMocks()
      await store2.deletePointsByFilePath(bsPath)
      const deleteFilter = mockTable.delete.mock.calls[0][0]
      expect(deleteFilter).toBe(`\`filePath\` IN ('${expectedKey}')`)
    })

    test("absolute inside-workspace path resolves to same key as scanner upsert", async () => {
      const filename = "src/components/Button.tsx"
      const payload = {
      ...
      resetAllMocks()
      await store2.upsertPoints(points)
      const storedKey = mockTable.add.mock.calls[0][0][0].filePath
      const normalized = path.normalize(filename)
      expect(storedKey).toBe(normalized)

      const absPath = path.resolve(wsPath, filename)
      resetAllMocks()
      await store2.deletePointsByFilePath(absPath)
      const deleteFilter = mockTable.delete.mock.calls[0][0]
      expect(deleteFilter).toBe(`\`filePath\` IN ('${storedKey}')`)
    })

    test("absolute path outside workspace produces relative key (defined behavior)", async () => {
      const outsidePath = path.resolve("C:").length >= 2 ? "C:\\Users\\outside\\file.ts" : "/tmp/outside/file.ts"
      mockTable.delete.mockResolvedValue(undefined)

      await store2.deletePointsByFilePath(outsidePath)

      const deleteFilter = mockTable.delete.mock.calls[0][0]
      expect(deleteFilter).toMatch(/`filePath` IN \('/)
    })
  })
})
